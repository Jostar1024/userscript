...
export default class extends React.PureComponent {
  ...
  static propTypes = {
    ...
    stylesheets: PropTypes.arrayOf(
      typeof window !== "undefined"
        ? PropTypes.instanceOf(window.CSSStyleSheet)
        : PropTypes.any,
    ),
  };
  ...
  componentDidMount() {
    const { delegatesFocus, mode, stylesheets } = this.props;

    const shadowRoot = this.placeholder.current.parentNode.shadowRoot;
    this.shadowRoot = shadowRoot || this.placeholder.current.parentNode.attachShadow({
      delegatesFocus,
      mode,
    });
  ...
  render() {
    if (!this.state.initialized) {
      if (this.props.declarative) {
        // @ts-ignore
        return (
          <template ref={this.placeholder} shadowroot={this.props.mode}>
            {this.props.children}
          </template>
        );
      }
